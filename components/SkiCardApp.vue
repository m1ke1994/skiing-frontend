...
// Список лыжных курсов из мок-данных
const courses = computed(() => mockDataStore.skiCourses);

const basketStore = useBasketStore();

// Добавляем выбранный курс в корзину
const addToCart = (course) => {
  basketStore.addToBasket(course);
  alert(`Курс «${course.title}» добавлен в корзину!`);
};

// Переключаем режим показа подробностей по курсу
const toggleDetails = (id) => {
  mockDataStore.toggleCourseDetails('skiCourses', id);
};
</script>

<template>
  <div class="course-container">
    <h1>Лыжные программы</h1>
